...
import com.example.pennydrop.data.PennyDropDatabase
import com.example.pennydrop.data.PennyDropRepository
...
class RankingsViewModel(application: Application):AndroidViewModel(application) {
    ...
    init {
        ...
        ) { statusesWithPlayers -> statusesWithPlayers.groupBy { it.player }
            .map { (player, statuses) ->
                PlayerSummary(
                    player.playerId,
                    player.playerName,
